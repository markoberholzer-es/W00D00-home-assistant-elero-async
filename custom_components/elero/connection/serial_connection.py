    ...
    async def open_connection(self) -> None:
        ...
        if not self.is_open():
            ...
            try:
                ...
                _LOGGER.debug("Failed to open serial port %s: %s", self._port_name, ex)
                await self.close()
                raise SerialConnectionError(
                    f"Failed to open serial port {self._port_name}"
                ) from ex


class SerialConnectionError(Exception):
    """Exception raised for serial connection errors."""
